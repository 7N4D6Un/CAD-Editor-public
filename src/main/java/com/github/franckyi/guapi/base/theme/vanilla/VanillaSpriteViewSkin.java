...
public class VanillaSpriteViewSkin extends AbstractSkin<SpriteView> {
    ...
    @Override
    public void render(SpriteView node, GuiGraphicsExtractor guiGraphicsExtractor, int mouseX, int mouseY, float delta) {
        super.render(node, guiGraphicsExtractor, mouseX, mouseY, delta);
        if (node.getSpriteFactory() != null) {
            RenderHelper.drawSprite(guiGraphicsExtractor, node.getSprite(), node.getX(), node.getY(), node.getImageWidth(), node.getImageHeight());
        }
    }
