...
public class EntityEntryModel extends ValueEntryModel<CompoundTag> {
    ...
    public void apply() {
        setValue(getValue());
        super.apply();
        this.defaultValue = copyValue();
    }

    @Override
    public void reset() {
        setValue(this.defaultValue == null || this.defaultValue.isEmpty() ? new CompoundTag() : this.defaultValue.copy());
    }
    ...
    private void syncFromValue(CompoundTag value) {
        ...
        if (type != null) {
            String canonical = EntityType.getKey(type).toString();
    ...
    private static CompoundTag sanitizeInitialValue(CompoundTag spawnData, EntityType<?> entityType) {
        ...
        if ((value == null || !value.contains("id") || value.getStringOr("id", "").isEmpty()) && entityType != null) {
            value.putString("id", EntityType.getKey(entityType).toString());
        }
        return value;
    }
    ...
    private CompoundTag sanitizeIncomingValue(CompoundTag incoming) {
        ...
        if (type != null) {
            ...
            value.putString("id", EntityType.getKey(getEntityType()).toString());
        }
        return value;
    }
    ...
    private static EntityType<?> findEntityType(String value) {
        ...
        return (EntityType) BuiltInRegistries.ENTITY_TYPE.getOptional(location).orElse(null);
    }
}
