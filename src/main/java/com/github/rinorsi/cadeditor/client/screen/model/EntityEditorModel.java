...
public class EntityEditorModel extends StandardEditorModel {
    ...
    private boolean isPlayerEntity() {
        ...
        if ("minecraft:player".equals(id) || "player".equals(id) || tag.contains("EnderItems") || tag.contains("abilities") || tag.contains("playerGameType") || tag.contains("recipeBook")) {
            return true;
        }
        if (tag.contains("components") && (components = tag.getCompound("components").orElse(null)) != null && components.contains("minecraft:profile")) {
            return true;
        }
        return false;
    }
    ...
    private boolean isTamable(Entity entity) {
        if (entity instanceof net.minecraft.world.entity.animal.camel.Camel) {
            // 骆驼不需要驯服
            return false;
        }
    ...
    private boolean isItemFrameLike(Entity entity) {
        EntityType<?> type;
        if (entity != null && ((type = entity.getType()) == EntityTypes.ITEM_FRAME || type == EntityTypes.GLOW_ITEM_FRAME)) {
            return true;
        }
        CompoundTag tag = getContext().getTag();
        if (tag == null) {
    ...
    public void handleEntityReplaced(CompoundTag newTag) {
        ...
        if (current != null) {
            copyListTag(current, copy, "Pos", 6);
            copyListTag(current, copy, "Rotation", 5);
            copyListTag(current, copy, "Motion", 6);
        ...
        if (!getCategories().isEmpty()) {
            EditorCategoryModel toSelect = getCategories().get(0);
            if (previousCategory != null) {
                for (EditorCategoryModel category : getCategories()) {
    ...
    private static void copyIntArrayTag(CompoundTag source, CompoundTag target, String key) {
        source.getIntArray(key).ifPresentOrElse(values -> {
            target.putIntArray(key, values);
        }, () -> {
            target.remove(key);
        });
    }
    ...
    private static void copyLongTag(CompoundTag source, CompoundTag target, String key) {
        source.getLong(key).ifPresentOrElse(value -> {
            target.putLong(key, value.longValue());
        }, () -> {
            target.remove(key);
        });
    }
