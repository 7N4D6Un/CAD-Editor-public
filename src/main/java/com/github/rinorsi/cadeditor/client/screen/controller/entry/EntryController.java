...
public abstract class EntryController<M extends EntryModel, V extends EntryView> extends AbstractController<M, V> {
    ...
    public void bind() {
        ...
        view.getUpButton().disableProperty().bind(model.listIndexProperty().eq(0).or(model.enabledProperty().not()));
        view.getDownButton().disableProperty().bind(model.listIndexProperty().eq(model.listSizeProperty().substract(1)).or(model.enabledProperty().not()));
        view.getDeleteButton().disableProperty().bind(model.enabledProperty().not());
        view.getResetButton().disableProperty().bind(model.enabledProperty().not());
